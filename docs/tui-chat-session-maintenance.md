...
- `tool_call` 事件展示
  ...
  - 当调用 `run_shell` 时，状态消息会额外展示 command 内容，便于用户感知实际执行命令。
  - 工具状态消息使用 AI 内容消息样式（Markdown + `message-ai`），不再使用 thinking 样式。
...
- `build_system_prompt()`（`src/ayu/system_prompt.py`）
  ...
  - 若当前工作目录存在 `AGENTS.md`，会把其内容拼接到 system prompt 末尾。
...
- `build_default_tool_registry()`（`src/ayu/tools.py`）
  ...
  - `run_shell` 定位为通用兜底工具：优先使用专用工具（如 `read_file`、`write_file`、`apply_patch`），仅在无专用工具可用时使用。
  - 当前内置识别示例：`git status/diff/log`（读目录）、`git add/commit`（写目录）、`grep`（读输入文件）、`find`（读搜索路径）、`curl`（按参数识别上传/下载文件）、`cp`（读源写目标）、`mv`（写源写目标）等。
  - 重定向会识别文件路径；文件描述符重定向（如 `2>&1`）不会误判为写文件路径。
